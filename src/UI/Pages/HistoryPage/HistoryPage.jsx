...
import { Navbar, HistoryVideos} from '../../../components';
import { useHistoryData } from '../../../hook';
import {deleteAllHistoryVideo} from '../../../utils/'
const HistoryPage = () => {

  const {historyState:{historyVideos}, historyDispatch} = useHistoryData();
     ...
     <Navbar/>
      <main  className='mt-3'>
        <section className="video-lists-container flex flex-wrap flex-between gap ">
          <h1>{(historyVideos.length===0)?'Watch history': `You have watched ${historyVideos.length} videos`}</h1>
          <button className="btn-primary btn btn-sm " onClick={clearHistory}>Clear History</button>
        </section>
        <section>
