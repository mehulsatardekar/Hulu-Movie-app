...
const VideoListingPage = () => {
    ...
    const filterVideos = (movieList)=>{
      return (type)=>{
       // const ds =arr.filter(v=>(type!== 'CLEAR_ALL')? v.types.includes(type): v);
        const filterMovies =  movieList.filter(movie=> (type===null)?movie : (type !== 'CLEAR_ALL') ? movie.types.includes(type):movie   )
        return filterMovies;
      }
    }
  ...
  return (
    ...
    <main  className='mt-3'>
      <section className="video-lists-container flex flex-wrap gap ">
        <button className="btn-outline-primary btn btn-sm btn-round-20  btn-py-1 pills "
         onClick={()=>{ setVideoCategory('Horror');}}>
