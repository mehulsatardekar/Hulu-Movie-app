...
const WatchLater = () => {
  ...
  const {watchLaterState:{watchLaterVideos}, watchLaterDispatch} = useWatchLaterData();
        ...
        return (
          ...
          >
            <div className="flex relative">
              ...
              />
              <span className="material-icons icon close-icon"
                 onClick={()=>{ deleteVideo(video._id ,video) }}
              >close</span>
            </div>
      ...
      {watchLaterVideos.length === 0 && (
        <>
          <div className="flex flex-column flex-align-item-center">
            ...
            <Link to="/" className="btn-primary btn btn-sm mt-3">Home</Link>
          </div>
        </>
      )}
    </>
